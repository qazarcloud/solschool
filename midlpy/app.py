...
@get("/account/{wallet:str}")
async def get_account(wallet: str) -> dict[str, id]:
    async with Client(
        transport=transport,
        ...
        result = await session.execute(query, variable_values={'address': wallet})
        print(result)
        return {"account": result}


app = Litestar([get_account])